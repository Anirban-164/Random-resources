...
Covered problems:
  ...
  Set 3 (tom.py):
    P1  — Classify systems (square, reverse) — linearity & time-invariance
    P2  — Distributivity / parallel connection: x*(h1+h2) = x*h1 + x*h2
    P3  — Cascade order independence: x*h1*h2 = x*h2*h1 = x*(h1*h2)
    P4  — BIBO stability bound: max|y| <= (Σ|h|) * max|x|
    P5  — Step response → first difference recovers h[n]
    P6  — Commutativity of convolution: x*h = h*x
    P7  — Convolution = polynomial multiplication
    P8  — Impulse decomposition (sifting property)
    P9  — Pure delay via delta[n-d]
    P10 — Decaying echo: h[n] = α^n u[n], recursion y[n] = x[n] + α·y[n-1]
"""
...
def scaled_shifted_impulse(start, end, position, height):
    """
    Return height · δ[n − position] over [start, end].
    (P8: sifting property — any signal = sum of scaled shifted impulses)
    """
    imp = DiscreteSignal(start, end)
...
def first_difference(sig):
    """
    r[n] = sig[n] − sig[n−1] over sig's range.
    (P5: differencing the step response recovers h[n])
    """
    out = DiscreteSignal(sig.start_time, sig.end_time)
    for n in sig.times():
        out.set_value_at_time(n,
            sig.get_value_at_time(n) - sig.get_value_at_time(n - 1))
    return out


def coefficient_at(a, b, n):
    """
    c[n] = Σ_k a[k] · b[n−k]  (single convolution sum term).
    (P7: convolution = polynomial multiplication)
    """
    total = 0.0
    for k in a.times():
...
# ── P3 (tom): Cascade — order independence ───────────────────────────
def cascade(first_system, second_system, x):
    """
    Apply first_system then second_system sequentially.
    x*h1*h2 = x*h2*h1 = x*(h1*h2)
    Trick: h1*h2 = LTISystem(h1).output(h2)
    """
    intermediate = first_system.output(x)
    return second_system.output(intermediate)


# ── P4 (tom): BIBO stability bound ──────────────────────────────────
def bibo_stability_check(h, x):
    """
    Verify: max|y| <= (Σ|h[k]|) · max|x|.
    Returns (stability_sum, peak_output, peak_input, bound_holds).
    """
    stability_sum = sum(abs(v) for v in h.values)
    y = LTISystem(h).output(x)
    peak_output = max(abs(y.get_value_at_time(n)) for n in y.times())
    peak_input  = max(abs(v) for v in x.values)
    bound_holds = peak_output <= stability_sum * peak_input + 1e-12
    return stability_sum, peak_output, peak_input, bound_holds


# ── P8 (tom): Impulse decomposition (sifting property) ──────────────
def impulse_decomposition(x):
    """
    Rebuild x[n] = Σ_k x[k] · δ[n−k] from scratch.
...
# ── P9 (tom): Pure delay system ─────────────────────────────────────
def pure_delay_system(d):
    """
    h[n] = δ[n−d]  ⟹  y[n] = x[n−d].
    Returns an LTISystem that delays by d samples.
    """
    h = make_signal(d, d, [1.0])
    return LTISystem(h)


# ── P10 (tom): Decaying echo impulse response ───────────────────────
def build_decay_impulse(alpha, N):
    """
    h[n] = α^n · u[n] truncated to n = 0..N.
    (Infinite impulse response approximated by finite truncation)
    """
    return make_signal(0, N, [alpha ** n for n in range(N + 1)])


def recursion_output(x, alpha, obs_end):
    """
    y[n] = x[n] + α · y[n−1]  for n = 0..obs_end.
    Equivalent to convolution with h[n] = α^n u[n].
    """
    y = DiscreteSignal(0, obs_end)
    for n in range(0, obs_end + 1):
        y.set_value_at_time(n,
            x.get_value_at_time(n) + alpha * y.get_value_at_time(n - 1))
    return y
...
def system_reverse(x):
    """y[n] = x[−n]  — linear, but time-VARYING (fails time-invariance)."""
...
def main():
    ...
    # ── 6. Cascade (order independence) ──────────────────────────────
    # forward = cascade(sys_a, sys_b, x)
    # reverse = cascade(sys_b, sys_a, x)
    # combined_h = LTISystem(h1).output(h2)  # h1*h2
    # direct = LTISystem(combined_h).output(x)

    # ── 7. BIBO stability ────────────────────────────────────────────
    # s_sum, peak_y, peak_x, ok = bibo_stability_check(h, x)

    # ── 8. First difference (recover h from step response) ───────────
    # s = sys.step_response(n_start, n_end)
    # recovered = first_difference(s)

    # ── 9. Polynomial multiplication ─────────────────────────────────
    # c = LTISystem(a).output(b)           # convolution = poly mult
    # c_n = coefficient_at(a, b, n)        # single term

    # ── 10. Impulse decomposition ─────────────────────────────────────
    # recon = impulse_decomposition(x)     # should == x

    # ── 11. Pure delay ────────────────────────────────────────────────
    # delay_sys = pure_delay_system(d)
    # y = delay_sys.output(x)              # should == x.shift(d)

    # ── 12. Decaying echo ─────────────────────────────────────────────
    # h = build_decay_impulse(alpha=0.5, N=15)
    # y_conv = LTISystem(h).output(x)
    # y_rec  = recursion_output(x, alpha=0.5, obs_end=10)
